...
import { deleteSeason } from '../api/seasons';
import { ApiError } from '../api/client';

interface DeleteSeasonConfirmModalProps {
  isOpen: boolean;
  onClose: () => void;
  showId: number;
  seasonToDelete: Season | null;
}

export const DeleteSeasonConfirmModal: React.FC<DeleteSeasonConfirmModalProps> = ({
  isOpen,
  onClose,
  showId,
  seasonToDelete,
  ...
  if (!isOpen || !seasonToDelete) return null;
  ...
  const isTrailer = seasonToDelete.season_number === 0;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in" role="dialog" aria-modal="true">
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden flex flex-col">
        ...
        {errorMessage && (
          <div className="m-6 mb-0 p-4 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-300 text-sm font-medium" role="alert" data-testid="delete-season-error">
            {errorMessage}
          </div>
        )}
        ...
        <div className="p-6 space-y-3">
          <p className="text-slate-300 leading-relaxed text-sm">
            ...
            <strong className="text-white font-semibold">
              {isTrailer ? 'Trailers' : `Season ${seasonToDelete.season_number}`}
            </strong>
            ?
          </p>
