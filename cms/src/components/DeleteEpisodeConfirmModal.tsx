...
interface DeleteEpisodeConfirmModalProps {
  isOpen: boolean;
  onClose: () => void;
  seasonId: number;
...
export const DeleteEpisodeConfirmModal: React.FC<DeleteEpisodeConfirmModalProps> = ({
  isOpen,
  onClose,
  seasonId,
  episodeToDelete,
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      await deleteEpisode(episodeToDelete.episode_id);
    },
    onSuccess: () => {
  ...
  if (!isOpen || !episodeToDelete) return null;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in" role="dialog" aria-modal="true">
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden flex flex-col">
        ...
        {errorMessage && (
          <div className="m-6 mb-0 p-4 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-300 text-sm font-medium" role="alert" data-testid="delete-episode-error">
            {errorMessage}
          </div>
        )}
        ...
        <div className="p-6 space-y-3">
          <p className="text-slate-300 leading-relaxed text-sm">
            ...
            <strong className="text-white font-semibold">{episodeToDelete.title}</strong> (
            <code className="bg-slate-950 px-2 py-0.5 rounded text-indigo-300 text-xs">{episodeToDelete.episode_id}</code> -{' '}
            <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-indigo-500/10 border border-indigo-500/30 text-indigo-300 uppercase tracking-wide">
              {episodeToDelete.language.toUpperCase()}
            </span>
            )?
          </p>
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 bg-slate-950/40 border-t border-slate-800">
          ...
          <button
            ...
            disabled={mutation.isPending}
            data-testid="confirm-delete-episode-btn"
          >
            {mutation.isPending ? (
